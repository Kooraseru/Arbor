...
That is the whole model. Runtime discovery and compile-time typing are separate concerns.
...
But only if the analyzer sees `script.Kick` and `script.Ban` from the file being checked.
...
`Arbor.LoadModules(root, validate)` is runtime behavior.

The runtime loader still calls `require` dynamically. Arbor keeps that dynamic value as `unknown` until your validator accepts it.

## Toolchain Notes

`luau-lsp` is recommended for development validation because it can show whether the active analyzer sees the tree you expect.

A source-map or data-model provider can help the analyzer see Roblox instance hierarchies. That provider belongs to your analyzer setup, not Arbor's runtime package.

## Next

- [Typechecking](guides/typechecking.md)
- [Runtime Loading](api/runtime-loaders/index.md)
