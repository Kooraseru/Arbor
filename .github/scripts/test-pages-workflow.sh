...
output_root="${1:-.generated-preview/pages/pages-workflow-test}"
branches=("${@:2}")
...
verify_publication_filter() {
	...
	for source_only_path in docs tools AGENTS.md; do
		if [ -e "$branch_path/$source_only_path" ]; then
			echo "Generated preview contains source-only path: $source_only_path" >&2
			exit 1
		fi
	done
}
...
set_site_metadata() {
	...
	if [ "$branch" = "preview" ]; then
		site_name_arg=(--site-name "Arbor Preview")
	fi
...
stage_docs_assets() {
	local branch_path="$1"
	local language="$2"
	local language_content_dir="$branch_path/content/locales/$language/wiki"
...
for branch in "${branches[@]}"; do
	...
	verify_publication_filter "$branch_path"
	...
	for language in en; do
		language_content_dir="$branch_path/content/locales/$language/wiki"
		if [ ! -d "$language_content_dir" ]; then
			echo "Skipping $branch $language docs: no content/locales/$language/wiki"
			continue
		fi

		language_config_path="$branch_path/.generated-preview/mkdocs-$language.yml"
		...
		(
			cd "$branch_path"
			"$python_cmd" -m mkdocs build --config-file ".generated-preview/mkdocs-$language.yml" --site-dir "$(python_path "$language_site_path")"
		)
	done
done
...
for rendered_example in \
	"$site_path/examples/serialized-types/index.html" \
	"$site_path/preview/examples/serialized-types/index.html"
do
	if [ -f "$rendered_example" ] && ! grep -Eq "Arbor/src/examples/serialized-types/IdentifyType.luau" "$rendered_example"; then
		echo "Rendered serialized-types example is missing included IdentifyType.luau source: $rendered_example" >&2
		exit 1
	fi
done
